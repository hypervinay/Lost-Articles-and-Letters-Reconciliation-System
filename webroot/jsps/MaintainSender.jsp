...
<html>
 ...
 <tr>
       <c:if test="${not empty MaintainSender}">
       <c:forEach var="SenderInfo" items="${MaintainSender}">
 ...
 %>
  ...
  <a href="<%=request.getContextPath()+"/MaintainLostPostAction"%>">Back</a>
  <%
  } else if(session.getAttribute("role").equals("postman"))
  {
  %>

  <a href="./jsps/PostManPosts.jsp?login=<%=(String)session.getAttribute("user")%>">Back</a>
  <%
  }
 %>
  
  <c:if test="${ empty SenderInfo}">
  <c:forEach var="SenderInfo" items="${SenderInfo}">
