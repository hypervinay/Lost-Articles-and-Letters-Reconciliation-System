...
-->
    ...
    <table width="637" border="0" align="center" bordercolor="#8692E3">
        ...
        <td width="372"><div align="center" class="style8">Solution</div></td>
         ...
         <c:if test="${not empty QueryInfo}">
         <c:forEach var="QueryInfo" items="${QueryInfo}">
       

      <tr bgcolor="#CcC9cA">
              <td bgcolor="#C3D7BA"><div align="center"><span class="style7">${QueryInfo.qid }</span></div></td>
        <td bgcolor="#C3D7BA"><div align="center" class="style7">
           
         ${QueryInfo.fname}${QueryInfo.lname }
         </div></td>
         <td>${QueryInfo.date }</td>
        
        
        
        <td bgcolor="#C3D7BA"><div align="center"><span class="style7">${QueryInfo.status }</span></div></td>
        <td bgcolor="#C3D7BA"><div align="center" class="style3">
        
        <c:choose>
        <c:when test="${QueryInfo.status eq 'inactive'}">
        
        <a href="./GetQueryAtAction?qid=${QueryInfo.qid }"> Replay</a>
        </c:when>
        </c:choose>
        
        <c:choose>
        <c:when test="${QueryInfo.status eq 'solved'}">
         Already Solved
      </c:when>
      </c:choose>
